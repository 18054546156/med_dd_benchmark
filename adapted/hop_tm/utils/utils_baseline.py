...
# 医疗数据集适配版本
# 修改日期: 2024-08-10
# 修改内容: 添加Kvasir数据集支持（PathMNIST和COVID已在原始代码中支持）
# 原始文件: raw/HoP-TM/utils/utils_baseline.py
...
# 统一 PathMNIST 的存储目录和标签标量化，其他 ImageFolder 分支保持官方结构。
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None, baseline=False):
    ...
    # HoP-TM 的 FTD/buffer 和蒸馏阶段必须读取同一份 prepared 数据。
    # 这里仅适配输入、标签和路径，high-order trajectory 逻辑保持原样。
    ...
    if medical_name is not None:
        ...
        dst_train = MedMNISTWrapper(PathMNIST(
            split='train', download=True, root=medmnist_root, transform=transform
        ))
        dst_test = MedMNISTWrapper(PathMNIST(
