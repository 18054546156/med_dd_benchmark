...
# 从仓库根目录引入统一医疗数据工具，避免 CAFE 与其他算法出现行为分叉。
...
def get_dataset(dataset, data_path):
    ...
    # 支持PathMNIST, COVID, Kvasir三个医疗影像数据集
        ...
        im_size = (32, 32)
        num_classes = 9
        mean = [0.741, 0.533, 0.706]  # 当前 benchmark 的工程归一化常数
        std = [0.402, 0.821, 0.407]   # MedMNIST INFO 不提供 mean/std

        # 创建数据变换：ToTensor -> Resize -> Normalize
        ...
        # 加载训练集和测试集
        # 注意：MedMNIST返回的标签是ndarray shape (1,)，需要标量化
        # 统一使用 data/PathMNIST；已有扁平缓存由工具函数兼容。
        medmnist_root = get_medmnist_root(data_path)
        raw_train = PathMNISTData(split='train', download=True, root=medmnist_root, transform=transform)
        raw_test = PathMNISTData(split='test', download=True, root=medmnist_root, transform=transform)

        # 使用包装器将标签从 array([5]) 转换为 5
        ...
        # 创建数据变换：Resize -> CenterCrop -> ToTensor -> Normalize
        transform = transforms.Compose([
            transforms.Resize(im_size, interpolation=transforms.InterpolationMode.BICUBIC),
        ...
        import os
        # 从共享 prepared/COVID 读取，train/val/test 由数据准备脚本固定。
        ...
        # Kvasir v2: 消化道内窥镜病变分类数据集
        # 来源: Simula Research Laboratory 官方 Kvasir 页面
        # 类别: 8类消化道异常类型
        # 图像尺寸: 128x128
        ...
        im_size = (128, 128)
        num_classes = 8
        # 这里使用 ImageNet 预训练模型约定，不代表 Kvasir 官方统计量。
        ...
        import os
        # Kvasir 只在这里适配数据接口，不改变 CAFE 的特征对齐逻辑。
