...
class Condenser:
    ...
    def condense(
        ...
        if getattr(args, "dam_enabled", False):
            ...
            use_final_feature_match = (
                getattr(args, "force_final_feature_match", False)
                or getattr(args, "net_type", "").lower() == "convnet"
            )
            inner_loss_fn = (
                match_loss
                if use_final_feature_match or args.depth <= 5
                else mutil_layer_match_loss
            )
